...
for i in comb_1:
    ...
    x_smt,y_smt=smt.fit_resample(x_train,y_train)
    scaler=StandardScaler()
    x_tain_scaled=scaler.fit_transform(x_smt)
    x_test_scaled=scaler.transform(x_test)
    import xgboost as xgb
...
for i in comb_2:
    ...
    x_test_scaled=scaler.transform(x_test)
    import xgboost as xgb
    xg=xgb.XGBClassifier(learning_rate=0.75,n_estimators=100,verbosity=0)
    xg.fit(x_train_scaled,y_smt)
    g=roc_auc_score(y_test,xg.predict_proba(x_test_scaled)[:, 1])
    auc2 .append(g)
...
comb_3=list(combinations(col[:-2],2))
auc3=[]
for i in comb_3:
    x=df.loc[:,1].values
    y=df.loc[:,col[-1]].values
...
comb_4=list(combinations(col[:-2],2))
auc4=[]
for i in comb_4:
    x=df.loc[:,1].values
    y=df.loc[:,col[-1]].values
...
comb_5=list(combinations(col[:-2],2))
auc5=[]
for i in comb_5:
    x=df.loc[:,1].values
    y=df.loc[:,col[-1]].values
...
comb_6=list(combinations(col[:-2],2))
auc6=[]
for i in comb_6:
    x=df.loc[:,1].values
    y=df.loc[:,col[-1]].values
...
comb_7=list(combinations(col[:-2],2))
auc7=[]
for i in comb_7:
    x=df.loc[:,1].values
    y=df.loc[:,col[-1]].values
...
comb_8=list(combinations(col[:-2],2))
auc8=[]
for i in comb_8:
    x=df.loc[:,1].values
    y=df.loc[:,col[-1]].values
...
comb_9=list(combinations(col[:-2],2))
auc9=[]
for i in comb_9:
    x=df.loc[:,1].values
    y=df.loc[:,col[-1]].values
...
comb_10=list(combinations(col[:-2],2))
auc10=[]
for i in comb_10:
    x=df.loc[:,1].values
    y=df.loc[:,col[-1]].values
...
comb_11=list(combinations(col[:-2],2))
auc11=[]
for i in comb_11:
    x=df.loc[:,1].values
    y=df.loc[:,col[-1]].values
